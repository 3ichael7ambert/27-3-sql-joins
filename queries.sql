...
SELECT
  owners.first_name,
  owners.last_name,
  COUNT(vehicles.owner_id) AS vehicle_count
FROM
  owners
LEFT JOIN
  vehicles ON owners.id = vehicles.owner_id
WHERE
  vehicles.owner_id IS NOT NULL
GROUP BY
  owners.first_name,
  owners.last_name
ORDER BY
...
SELECT
  owners.first_name,
  owners.last_name,
  AVG(ROUND(vehicles.price))::INTEGER AS average_price,
  COUNT(vehicles.owner_id) AS vehicle_count
FROM
  owners
JOIN
...
HAVING
  ...
  AND AVG(ROUND(vehicles.price)) > 10000
ORDER BY
